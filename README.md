...
- Para marcar automaticamente respostas novas em `form_answered`, configure no Tally do formulário `wdRX0N` um webhook `POST` para `https://agendamento-ia-reunioes-6b454.shrd00.internal.goskip.dev/backend/v1/tally/webhook`. O secret do webhook deve ser o mesmo valor salvo em `TALLY_SIGNING_SECRET`.
- Para validar respostas antigas, configure `TALLY_API_KEY`. Ao consultar um cliente que ainda está com `form_answered = false`, o backend busca as submissões completas do formulário `wdRX0N` na API do Tally, extrai todos os emails encontrados nas respostas/campos ocultos e atualiza os clientes correspondentes para `form_answered = true`. Admins também podem disparar a sincronização manualmente em `POST /backend/v1/tally/sync`.
