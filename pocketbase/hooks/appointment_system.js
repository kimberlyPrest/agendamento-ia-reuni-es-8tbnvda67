...
function bad(e, message) {
  return e.json(400, { error: message, message })
}

function forbidden(e) {
  return e.json(403, { error: 'Acesso restrito ao admin.', message: 'Acesso restrito ao admin.' })
}

function notFound(e, message) {
  return e.json(404, { error: message, message })
}
...
function getDayKey(dateStr) {
  ...
  return { dow, key: DAY_KEYS[dow] }
}

function parseWorkingHours(consultant) {
...
function getRawDaySchedule(consultant, dateStr) {
  const wh = parseWorkingHours(consultant)
  const day = getDayKey(dateStr)
  return wh[day.key] || wh[String(day.dow)] || wh[day.dow] || []
}

function normalizeDaySchedule(raw, duration) {
  const windows = []
  const exactTimes = []
  ...
  if (windows.length === 0 && exactTimes.length === 0) {
...
function recordTime(record, field) {
  return parseDate(record.get(field))
}
...
function getProgramLimit(client, program) {
...
function getClientStats(client, program) {
  const now = new Date()
...
function getUpcomingMeeting(clientId) {
  try {
    ...
    const meeting = $app.findFirstRecordByFilter(
      'meetings',
      `client_id = '${clientId}' && status = 'scheduled' && start_time > '${now}'`,
      'start_time',
    )
    return expandMeeting(meeting)
  } catch (_) {
    return null
  }
}

function getLocalBusyIntervals(consultantId, dateStr, ignoreMeetingId) {
...
function googleConnected(consultant) {
  return Boolean(googleConfigReady() && consultant.get('google_refresh_token'))
}
...
function refreshGoogleAccessToken(consultant) {
...
function googleFreeBusy(consultant, timeMin, timeMax) {
  ...
  if (!calendar || calendar.errors) throw new Error('Calendário Google inválido ou sem permissão.')
...
function buildSlots(consultant, program, dateStr, ignoreMeetingId) {
  ...
  const googleBusy = googleConnected(consultant)
    ? googleFreeBusy(consultant, startOfDay, endOfDay)
    : []
...
function assertBusinessRules(client, consultant, program, start, end, ignoreMeetingId) {
  ...
  if (minInterval > 0 && stats.last_meeting) {
    ...
    const allowedAfter = addInterval(
      lastEnd,
      minInterval,
      textValue(program, 'min_interval_unit', 'days'),
    )
    if (start < allowedAfter) {
  ...
  const duration = numberValue(program, 'meeting_duration', 60)
  const expectedEnd = addMinutes(start, duration)
  if (Math.abs(expectedEnd.getTime() - end.getTime()) > 60000) {
  ...
  const hasLocalConflict = localBusy.some((busy) =>
    intervalsOverlap(
      addMinutes(start, -bufferBefore),
      addMinutes(end, bufferAfter),
      busy.start,
  ...
  if (hasLocalConflict) return 'Horário já reservado.'

  if (!googleConnected(consultant)) {
    return 'Agenda Google do consultor ainda não está conectada por OAuth.'
  }

  const googleBusy = googleFreeBusy(
    consultant,
    addMinutes(start, -bufferBefore),
    addMinutes(end, bufferAfter),
  )
  const hasGoogleConflict = googleBusy.some((busy) =>
    intervalsOverlap(
      addMinutes(start, -bufferBefore),
      addMinutes(end, bufferAfter),
      busy.start,
  ...
  if (hasGoogleConflict) return 'Este horário está ocupado na agenda Google do consultor.'
...
function buildMeetingTitle(program, client, meetingNumber) {
...
function createGoogleEvent(consultant, client, program, title, start, end) {
  const accessToken = refreshGoogleAccessToken(consultant)
  if (!accessToken) throw new Error('Google Calendar não conectado.')
  const calendarId = consultant.get('google_calendar_id') || 'primary'
  const requestId = $security.randomString(24).toLowerCase()
  const res = $http.send({
...
function patchGoogleEvent(consultant, meeting, title, start, end) {
...
function deleteGoogleEvent(consultant, meeting) {
  const eventId = meeting.get('google_event_id')
  if (!eventId) return
  const accessToken = refreshGoogleAccessToken(consultant)
  if (!accessToken) return
...
function canChangeMeeting(meeting, program) {
  const start = recordTime(meeting, 'start_time')
  const minHours = numberValue(program, 'min_reschedule_hours', 24)
  return start && start.getTime() - Date.now() >= minHours * 60 * 60 * 1000
}

routerAdd('POST', '/backend/v1/client/auth', (e) => {
  const body = e.requestInfo().body || {}
  if (!body.email) return bad(e, 'Email é obrigatório')
  ...
  try {
    ...
    return notFound(e, 'Email não encontrado')
  }
})
...
routerAdd('GET', '/backend/v1/calendar/slots', (e) => {
  ...
  if (!consultantId || !dateStr) return bad(e, 'Parâmetros obrigatórios ausentes')
  ...
  try {
    ...
    if (clientId) {
      ...
      program = { get: (field) => (field === 'meeting_duration' ? 60 : 0) }
    }
    ...
    const slots = buildSlots(consultant, program, dateStr, ignoreMeetingId)
    return e.json(200, {
      slots,
    ...
    return bad(e, err.message || 'Erro ao buscar horários')
  }
})
...
routerAdd('POST', '/backend/v1/calendar/book', (e) => {
  ...
  const client = clientId ? $app.findRecordById('clients', clientId) : null
  if (!client) return bad(e, 'Cliente inválido')
  ...
  try {
    ...
    if (!start || !end) return bad(e, 'Horário inválido')

    const ruleError = assertBusinessRules(client, consultant, program, start, end, '')
    if (ruleError) return bad(e, ruleError)
    ...
    return bad(e, err.message || 'Erro ao agendar')
  }
})
...
routerAdd('POST', '/backend/v1/calendar/cancel', (e) => {
  ...
  const clientId = body.client_id || body.clientId
  if (!meetingId) return bad(e, 'Agendamento inválido')

  try {
    const meeting = $app.findRecordById('meetings', meetingId)
    if (!clientId && !requireAdmin(e)) return forbidden(e)
    if (clientId && meeting.get('client_id') !== clientId && !requireAdmin(e)) return forbidden(e)
    ...
    if (!canChangeMeeting(meeting, program)) {
      return bad(e, 'Cancelamentos e remarcações exigem no mínimo 24h de antecedência.')
    }
    ...
    return bad(e, err.message || 'Erro ao cancelar')
  }
})

routerAdd('POST', '/backend/v1/calendar/reschedule', (e) => {
  const body = e.requestInfo().body || {}
  const meetingId = body.meeting_id || body.meetingId
  const clientId = body.client_id || body.clientId
  if (!meetingId) return bad(e, 'Agendamento inválido')
  ...
  try {
    ...
    const title =
      meeting.get('title') || buildMeetingTitle(program, client, meeting.get('meeting_number') || 1)
    ...
    if (googleEvent) {
      meeting.set('google_html_link', googleEvent.htmlLink || meeting.get('google_html_link') || '')
      meeting.set('meet_link', getMeetLinkFromEvent(googleEvent) || meeting.get('meet_link') || '')
    }
    $app.save(meeting)
    return e.json(200, { meeting: expandMeeting(meeting) })
  } catch (err) {
    return bad(e, err.message || 'Erro ao remarcar')
  }
})

routerAdd(
  'GET',
  '/backend/v1/google/oauth/start',
  (e) => {
    if (!requireAdmin(e)) return forbidden(e)
    const consultantId =
      e.request.url.query().get('consultant_id') || e.request.url.query().get('consultantId')
    if (!consultantId) return bad(e, 'Consultor obrigatório')
    ...
    try {
      ...
      const url = `${GOOGLE_AUTH_URL}?${formEncode({
        client_id: env('GOOGLE_CLIENT_ID'),
      ...
      return bad(e, err.message || 'Erro ao iniciar OAuth')
    }
  },
  $apis.requireAuth('users'),
)

routerAdd('GET', '/backend/v1/google/oauth/callback', (e) => {
  const code = e.request.url.query().get('code')
  const state = e.request.url.query().get('state')
  if (!code || !state) return bad(e, 'Callback OAuth inválido')

  try {
    const consultantId = state.split(':')[0]
    const consultant = $app.findRecordById('consultants', consultantId)
    if (consultant.get('google_oauth_state') !== state) return bad(e, 'State OAuth inválido')
    ...
    if (tokenRes.statusCode < 200 || tokenRes.statusCode >= 300) {
      return bad(e, 'Google recusou a autorização.')
    }
    ...
    return e.html(
      200,
      '<!doctype html><meta charset="utf-8"><title>Google conectado</title><body style="background:#0A0A0A;color:#fff;font-family:DM Sans,Arial,sans-serif;display:grid;place-items:center;min-height:100vh"><main style="max-width:520px;text-align:center"><h1 style="font-family:Syne,Arial,sans-serif;color:#FF6B00">Google Calendar conectado</h1><p>Você já pode voltar ao admin do Agendamentos Elite.</p></main></body>',
    )
  } catch (err) {
    return bad(e, err.message || 'Erro no callback OAuth')
  }
})
...
routerAdd('POST', '/backend/v1/tally/webhook', (e) => {
  ...
  if (secret) {
    ...
    const calculated = String($security.hs256(JSON.stringify(body), secret)).replace('sha256=', '')
